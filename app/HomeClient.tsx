...
export default function HomeClient({
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 z-50 w-80 bg-gray-100 dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 p-6 overflow-y-auto transform transition-transform duration-300 ease-in-out
      ...
      >
        ...
        <form onSubmit={handleSearchSubmit} className="mb-6 relative">
          <input
            ...
            className="w-full bg-white dark:bg-gray-700 p-3 pr-10 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-300 outline-none text-sm dark:text-gray-100"
            type="text"
            placeholder="Search..."
          />
          <button
          ...
          >
            <Search className="text-gray-400 dark:text-gray-500 w-5 h-5 hover:text-blue-500" />
          </button>
        </form>
            ...
            <ul className="space-y-2 text-sm">
              {categories.map((cat, i) => (
                <li
                  ...
                  className={`cursor-pointer px-2 py-1 rounded hover:bg-blue-50 dark:hover:bg-blue-900/30 hover:text-blue-600 dark:hover:text-blue-400 transition-colors flex justify-between items-center
                    ${currentCategory === cat ? "text-blue-600 dark:text-blue-400 font-bold bg-blue-50 dark:bg-blue-900/30" : "text-gray-600 dark:text-gray-300"}`}
                >
                  <span>{cat}</span>
                  {currentCategory === cat && (
            ...
            <ul className="space-y-2 text-sm">
              {seriesList.map((series, i) => (
                <li
                  ...
                  className={`cursor-pointer px-2 py-1 rounded hover:bg-purple-50 dark:hover:bg-purple-900/30 hover:text-purple-600 dark:hover:text-purple-400 transition-colors flex justify-between items-center
                    ${currentSeries === series ? "text-purple-600 dark:text-purple-400 font-bold bg-purple-50 dark:bg-purple-900/30" : "text-gray-600 dark:text-gray-300"}`}
                >
                  <span>{series}</span>
                  {currentSeries === series && (
      ...
      <main className="flex-1 w-full p-4 md:p-8 overflow-y-auto">
        ...
        <div className="md:hidden flex justify-between items-center mb-6">
          <button
            onClick={toggleMenu}
            className="p-2 -ml-2 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg"
          >
            <Menu className="w-6 h-6 text-gray-700 dark:text-gray-300" />
          </button>
          <span className="font-bold text-gray-700 dark:text-gray-300"></span>
          <button onClick={clearFilters}>
            <LucideHome className="w-6 h-6 text-gray-700 dark:text-gray-300" />
          </button>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {!isFiltering && (
              <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Recent Posts</h1>
            )}
          </div>
          ...
          {!currentSeries && (
            <button
              onClick={toggleSort}
              className="flex items-center gap-2 px-3 py-1.5 cursor-pointer bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-sm font-medium text-gray-600 dark:text-gray-300 transition-colors"
            >
        ...
        >
          {filteredAndSortedPosts.length > 0 ? (
            filteredAndSortedPosts.map((post) => (
              <div
                key={post.id}
                className="flex flex-col h-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
              >
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    {post.series_name && (
                      <span className="text-[10px] font-bold text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/30 px-2 py-0.5 rounded-full truncate max-w-[100px]">
                  ...
                  <Link href={`/posts/${post.slug}`} className="block group">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-2 mb-2">
                      {post.title}
                    </h2>
                  </Link>
